...
pub fn part2() -> usize {
    ...
    // by subtracting the common planets we'll be left with a path to a common branch point, then we can count steps from YOU/SAN to common
    let common_planets = you_path.intersection(&santa_path).count();

    // -2 because the YOU and SAN planets themselves don't count
    let result = (you_path.len() - common_planets) + (santa_path.len() - common_planets) - 2;
    result
}
